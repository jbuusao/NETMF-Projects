/*
 * $Id: HttpWiflyImpl.cs 6 2012-04-04 12:11:18Z jbuusao $
 * 
...
namespace astra.http
{
    public class HttpWiflyImpl : HttpImplementation, IDisposable
    {
        ...
        private byte[] serialReadBuffer = new byte[1];
        ...
        public HttpResponse SendRequest(String host, int port, String data, Boolean parseResponse)
        {
            /*

set com remote POST$/api/feeds/18992.csv?_method=put
set option format 0
set ip proto 18
open www.pachube.com 80
Accept: * / *
Content-Length: 9
Host: www.pachube.com
User-Agent: NetduinoPlus
X-PachubeApiKey: 5jm9fvg00nhhHreaYGmh0YWu479knwoMbSOMz_Rnwe4
...
20,21,22
            ...
            enterCommandMode();
            Thread.Sleep(100);
            //SendCommand("set com remote POST$/api/feeds/18992.csv?_method=put\n");
            SendCommand("set com remote GET$/\n");
            SendCommand("set option format 1\n");
            //SendCommand("set ip proto 18\n");
            //
            SendCommand("open www.secretlabs.com 80\n");

            //SendCommand("Accept: */*\n");
            //SendCommand("Content-Length: "+data.Length+"\n");
            //SendCommand("Host: www.pachube.com\n");
            //SendCommand("User-Agent: NetduinoPlus\n");
            //SendCommand("X-PachubeApiKey: 5jm9fvg00nhhHreaYGmh0YWu479knwoMbSOMz_Rnwe4\n\n");
            ...
            while (true)
            {
                ...
                if (contents.Length != 0 && parseResponse)
                {
        ...
        public void Listen()
        {
            ...
            while (true)
            {
                ...
                if (m_serialPort != null && m_serialPort.Read(serialReadBuffer, 0, serialReadBuffer.Length) > 0)
                    WriteRegister(WiflyRegister.THR, serialReadBuffer[0]);
